...
# ==================== n-p pair scan ====================
def evaluate_pair(a_p, b_p, a_n, b_n, c, Tc, Th, model1, model2, sigma_fn1, Seebeck_fn1, sigma_fn2, Seebeck_fn2, stats1, stats2):
...
def dimension_scan_two(model1, model2, sigma_fn1, Seebeck_fn1,
                       sigma_fn2, Seebeck_fn2, stats1, stats2,
                       Tc, Th, Apn_total, ratio_list, H_ratios):

    start_time = time.time()
    Ap_An_list, H_Apn_list, P_max_list, eff_max_list = [], [], [], []
    for ratio in ratio_list:
        Ap = Apn_total * ratio / (1 + ratio)
        An = Apn_total - Ap
        ...
        for H_ratio in H_ratios:
            ...
            P_max, eff_max = evaluate_pair(a_p, b_p, a_n, b_n, c,
                                           Tc, Th, model1, model2,
                                           sigma_fn1, Seebeck_fn1,
                                           sigma_fn2, Seebeck_fn2,
                                           stats1, stats2)

            Ap_An_list.append(ratio)
    ...
    end_time = time.time()
    print(f"[dimension_scan_two] Th = {Th:.2f}, Time used: {end_time - start_time:.4f} seconds")

    return Ap_An_list, H_Apn_list, P_max_list, eff_max_list
...
model2 = load_model(path=os.path.join(model_dir, 'SnS.pth'))

data_dir = os.path.join(parent_dir, "data")
csv_file1 = os.path.join(data_dir, "Mg3Sb2.csv")
csv_file2 = os.path.join(data_dir, "SnS.csv")
...
material_csv2 = os.path.join(property_dir, "SnS_property.csv") 
...
ratio_list = np.geomspace(0.2, 5, 20)
H_ratios = np.linspace(0.1, 0.5, 20)
Ap_An_list, H_Apn_list, P_max_list, eff_max_list = dimension_scan_two(model1, model2, sigma_fn1, Seebeck_fn1, sigma_fn2, Seebeck_fn2, 
                                                                      stats1, stats2, Tc, Th, Apn_total, ratio_list, H_ratios)

export_surface_csv(Ap_An_list, H_Apn_list, P_max_list, "P_max_surface_Mg3Sb2_SnS.csv")
export_surface_csv(Ap_An_list, H_Apn_list, eff_max_list, "efficiency_surface_Mg3Sb2_SnS.csv")
