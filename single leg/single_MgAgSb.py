...
def get_precise_max(I_list, y_list):
    if len(I_list) < 4:
        return None, None
    I_array = np.array(I_list)
    y_array = np.array(y_list)

    cs = CubicSpline(I_array, y_array)

    I_fine = np.linspace(I_array.min(), I_array.max(), 500)
    y_fine = cs(I_fine)

    max_index = np.argmax(y_fine)
    return I_fine[max_index], y_fine[max_index]
...
def scan_current_sweep(model, a, b, c, Tc, Th, I_range, stats):
    ...
    I_P_max, P_max = get_precise_max(I_list, P_list)
    I_eff_max, eff_max = get_precise_max(I_list, efficiency_list)
...
# ==================== Temperature scan ====================
def temperature_param_scan(model, Tc, Th_list, a, b, c, sigma_fn, Seebeck_fn, stats):
    start_time = time.time()
    ...
    for Th in Th_list:
        ...
        I_range = compute_ih_range(a, b, c, Tc, Th, sigma_fn, Seebeck_fn, ih_count=20)

        V_list, q_list, P_list, eff_list, I_list, P_max, eff_max = scan_current_sweep(
            model, a, b, c, Tc, Th, I_range, stats
        )
    ...
    end_time = time.time()
    print(f"[temperature_param_scan] Time used: {end_time - start_time:.4f} seconds")

    return Th_valid, P_max_list, eff_max_list


# ==================== Export csv function ====================
def export_current_scan_csv(filename, I_list, V_list, q_list, P_list, efficiency_list):
    df = pd.DataFrame({
        'Current (A)': I_list,
        'Voltage (V)': V_list,
        'Heat Flux (W)': q_list,
    ...
    print(f"[Saved] Current scan data exported to: {filename}")

def export_temperature_scan_csv(filename, Th_valid, P_max_list, eff_max_list):
    df = pd.DataFrame({
        'Hot-side Temperature Th (K)': Th_valid,
        'Max Power (W)': P_max_list,
        'Max Efficiency (%)': eff_max_list
    })
    df.to_csv(filename, index=False)
    print(f"[Saved] Temperature scan data exported to: {filename}")
...
model = load_model(path=os.path.join(model_dir, 'MgAgSb.pth'))

data_dir = os.path.join(parent_dir, "data")
csv_file = os.path.join(data_dir, "MgAgSb.csv")

_, _, stats, _ = load_dataset_from_csv(csv_file)

property_dir = os.path.join(parent_dir, "property")
material_csv = os.path.join(property_dir, "MgAgSb_property.csv") 

df = pd.read_csv(material_csv)
material_property = [
    list(zip(df['T'], df['sigma'])),
    list(zip(df['T'], df['kappa'])),
    list(zip(df['T'], df['Seebeck']))
]

sigma_fn, kappa_fn, Seebeck_fn = get_material_property_fns(material_property)

# ==================== Current scan ====================
a_fix, b_fix, c_fix, Tc_fix, Th_fix = 3.5, 3.5, 7, 293.15, 573.15
I_range = compute_ih_range(a_fix, b_fix, c_fix, Tc_fix, Th_fix, sigma_fn, Seebeck_fn, ih_count=20)
V_list, q_list, P_list, efficiency_list, I_list, P_max, eff_max = scan_current_sweep(model, a_fix, b_fix, c_fix, Tc_fix, Th_fix, I_range, stats)

export_current_scan_csv('I_scan_MgAgSb_Th_573.csv', I_list, V_list, q_list, P_list, efficiency_list)

# ==================== Temperature scan ====================
Tc = 293.15
Th_list = np.linspace(313.15, 573.15, 14)
a, b, c = 3.8, 3.8, 7
Th_valid, P_max_list, eff_max_list = temperature_param_scan(model, Tc, Th_list, a, b, c, sigma_fn, Seebeck_fn, stats)

export_temperature_scan_csv('T_scan_MgAgSb.csv', Th_valid, P_max_list, eff_max_list)
